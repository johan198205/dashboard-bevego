            ...
            {data && (
              <div className="space-y-6">
                ...
                {data.breakdownRows.length > 0 && (
                  <div>
                    <h3 className="font-semibold text-dark dark:text-white mb-3">
                      Nedbrytningsrader ({data.breakdownRows.length})
                    </h3>
                    <div className="space-y-2 max-h-64 overflow-y-auto">
                      {data.breakdownRows.slice(0, 10).map((row, index) => (
                        <div key={index} className="rounded-lg bg-gray-50 dark:bg-gray-800 p-3">
                          <div className="flex items-center justify-between">
                            <div>
                              <div className="font-medium text-dark dark:text-white">
                                {row.groupA || row.groupB || row.groupC || `Rad ${index + 1}`}
                              </div>
                              {row.weight > 0 && (
                                <div className="text-sm text-gray-600 dark:text-gray-400">
                                  Vikt: {row.weight.toLocaleString('sv-SE')}
                                </div>
                              )}
                            </div>
                            <div className="text-lg font-semibold text-primary">
                      ...
                      {data.breakdownRows.length > 10 && (
                        <div className="text-center text-sm text-gray-600 dark:text-gray-400 py-2">
                          ... och {data.breakdownRows.length - 10} fler rader
                        </div>
                      )}
                    </div>
                  </div>
                ...
                {/* Changes */}
                {(data.qoqChange !== undefined || data.yoyChange !== undefined) && (
                  <div>
                    <h3 className="font-semibold text-dark dark:text-white mb-3">
                      Förändringar
                    </h3>
                    <div className="space-y-2">
                      {data.qoqChange !== undefined && (
                        <div className="flex items-center justify-between rounded-lg bg-gray-50 dark:bg-gray-800 p-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-dark dark:text-white">QoQ</span>
                          </div>
                      ...
                      {data.yoyChange !== undefined && (
                        <div className="flex items-center justify-between rounded-lg bg-gray-50 dark:bg-gray-800 p-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-dark dark:text-white">YoY</span>
                          </div>
                          <span className={`font-semibold ${
